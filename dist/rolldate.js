...
(function (global, factory) {
  ...
  @media screen and (max-width:414px){
      .rolldate-container{font-size:27px}
  }
  ...
  Rolldate.prototype = {
      ...
      baseData: function baseData() {
          return {
              ...
              opts: { //插件默认配置
                  ...
                  minStep: 1,
                  trigger: 'tap',
                  lang: { title: 'Tanggal Mulai', cancel: 'Batal', confirm: 'Simpan', year: '', month: '', day: '', hour: '', min: '', sec: '' }
              }
          };
      },
      ...
      createUI: function createUI() {
          ...
          for (var i = 0; i < len; i++) {
			  ...
			  const thebeginYear = config.beginYear || date.getFullYear();
			  const thebeginDay = config.beginDay || 1;
			  const thebeginMonth = config.beginMonth || 1;
			  
              if (f == 'YYYY') {
                  for (var j = thebeginYear; j <= (config.endYear || thebeginYear+2); j++) {
                      itemClass = thebeginYear ? 'active' : '';
                  ...
                  for (var k = thebeginMonth; k <= (config.endMonth || 12); k++) {
                      itemClass = thebeginMonth ? 'active' : '';
                  ...
                  for (var l = thebeginDay; l <= (config.endDay || day); l++) {
                      itemClass = thebeginDay ? 'active' : '';
